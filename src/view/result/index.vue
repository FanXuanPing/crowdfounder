<template>
	<div>
		...
		<div class="resultDiv" infinite-scroll-immediate-check='true' v-infinite-scroll="loadMore" infinite-scroll-disabled="loading"  infinite-scroll-distance="10">
			...
			<div v-for="item in resultList" class="resultList" @click="goToprize(item.id,item.state)">
				<div class="resultTop">
					<div class="coinImg">
				...
				<div class="resultBot">
					...
					<div class="ckz" @click.stop="ckz(item.referencePlatform,item.referenceCount,item.referenceDate,item.referencePrice,item.referenceType,item.referenceCount,item.referencePrice,item.total,item.lucky)">参考值</div>
				</div>  
				...
				<div class="referDiv" v-if="shows">
					<div class="refer">
						...
						<button class="iknow" @click.stop="iknow()">
							我知道了
						</button>
					</div>
				</div>
